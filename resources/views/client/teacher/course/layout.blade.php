            ...
            <div class="w-full bg-primary-20/25 h-60">
                <div class="container relative z-10 flex flex-col justify-between h-full pt-24 mx-auto">
                    ...
                    <h2 class="flex items-center text-2xl font-medium font-ibm text-secondary">
                        <a href="{{ route('client.auth.teacher.all') }}" class="inline-block mr-3 leading-none"
                    ...
                    <div>
                        <ul
                            ...
                            <li>
                                <a href="{{ route('client.auth.teacher.course.manage.overview') }}" aria-current="page"
                                    class="inline-block p-4  rounded-t-xl
                                    {{ in_array(Route::currentRouteName(), ['client.auth.teacher.course.manage.overview', 'client.auth.teacher.course.manage.learn', 'client.auth.teacher.course.manage.requirements']) ? 'text-primary bg-primary-20/30' : 'hover:text-gray-600 hover:bg-white' }}">
                                    <i class="text-lg leading-none fi fi-rr-home"></i>
                                    <span class="block mt-1.5">ส่วนหน้า</span>
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('client.auth.teacher.course.content.index') }}"
                                    class="inline-block p-4 rounded-t-xl   {{ in_array(Route::currentRouteName(), ['client.auth.teacher.course.content.index', 'client.auth.teacher.course.content.form']) ? 'text-primary bg-primary-20/30' : 'hover:text-gray-600 hover:bg-white' }}">
                                    <i class="text-lg leading-none fi fi-rr-book-alt"></i>
                                    <span class="block mt-1.5">เนื้อหา</span>
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('client.auth.teacher.course.settings') }}"
                                    class="inline-block p-4 rounded-t-xl {{ Route::currentRouteName() == 'client.auth.teacher.course.settings' ? 'text-primary bg-primary-20/30' : 'hover:text-gray-600 hover:bg-white' }}">
